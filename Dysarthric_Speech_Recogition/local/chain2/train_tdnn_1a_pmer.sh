...
# steps/info/chain_dir_info.pl exp_${feat}/chain2/tdnn1a_sp
# exp_${feat}/chain2/tdnn1a_sp: num-iters=6 nj=2..5 combine=-0.038->-0.033 (over 3)

# local/chain2/compare_wer.sh exp_${feat}/chain2/tdnn1a_sp
# System                tdnn1a_sp
...
# run those things.
local/chain2/train_ivector_tdnn_1a_pmer.sh --stage $stage \
				  --feat $feat \
                                  --train-set $train_set \
                                  --gmm $gmm \
...
tree_dir=exp_${feat}/chain2${nnet3_affix}/tree_sp${tree_affix:+_$tree_affix}
lang=data_${feat}/lang_chain
lat_dir=exp_${feat}/chain2${nnet3_affix}/${gmm}_${train_set}_sp_lats
dir=exp_${feat}/chain2${nnet3_affix}/tdnn${affix}_sp
train_data_dir=data_${feat}/${train_set}_sp_hires
lores_train_data_dir=data_${feat}/${train_set}_sp
train_ivector_dir=exp_${feat}/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires
...
if [ $stage -le 10 ]; then
  ...
  if [ -d $lang ]; then
    if [ $lang/L.fst -nt data_${feat}/lang/L.fst ]; then
      echo "$0: $lang already exists, not overwriting it; continuing"
    else
      echo "$0: $lang already exists and seems to be older than data_${feat}/lang..."
      echo " ... not sure what to do.  Exiting."
      exit 1;
    fi
  else
    cp -r data_${feat}/lang $lang
...
if [ $stage -le 11 ]; then
  ...
  steps/align_fmllr_lats.sh --nj $decode_nj --cmd "$train_cmd" ${lores_train_data_dir} \
    data_${feat}/lang $gmm_dir $lat_dir
